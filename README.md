...
## Why moored ships no longer flag

The first serious use of this exposed the defect that made it unusable: berthed and anchored vessels firing as `loiter`. That was not a tuning problem, it was a category error. Cable routes terminate at landing stations, landing stations sit beside ports, and a 3 km corridor swallows whole harbours. Every ship at a berth inside one satisfies "under two knots for two hours" perfectly. The detector worked exactly as specified; the specification was wrong.

Four independent guards now suppress it, any one of which is sufficient:

**Geometry.** A hull made fast to a pier moves by GPS noise alone — tens of metres of scatter. That case is decidable with no external data at all and is rejected outright.

**Confinement plus self-report.** A vessel reporting *At anchor* whose track stays inside a circle is riding to its anchor, which is what anchors are for. Note the asymmetry that makes this safe: **a hull dragging its anchor moves**, so it fails the confinement test and still reports, even while broadcasting *At anchor*. What gets suppressed is anchored-and-staying-put.

**Seeded locations.** 1,081 Natural Earth ports and 1,335 cable landing points, bundled, offline, working on run one.

**Learned locations.** The real fix, and it needs no dataset. A place where many *different* vessels have sat still is an anchorage by definition. GRAPNEL accumulates a grid of distinct MMSIs observed stationary per cell and suppresses there. This self-calibrates to every berth, roadstead, waiting area and fishing ground on earth, and improves the longer it runs. Forty stationary ships in one cell are a harbour, not forty saboteurs.

What deliberately still fires: a vessel holding station over a cable *somewhere vessels are not observed to stop*. That is not a bug — it is the *Hong Tai 58* case, which held station over the Taiwan–Penghu cable before it was cut. Geometrically that is identical to an anchored ship. Only location separates them, which is exactly why suppression is location-based rather than geometric.

The suppression grid is written to `data/stopping-places.json` and the counts published in `detections.json`, so anything suppressed is auditable rather than silently dropped.

One honest weakness: a location where sabotage happened repeatedly would eventually be learned as normal. That needs many distinct hulls loitering in the same cell over weeks, so it is theoretical rather than practical — but it is real, and it is why the counts are published.
